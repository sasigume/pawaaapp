...
import { getPlatform, getAllPlatformsWithSlug, getAllPostsForPlatform } from '@/lib/contentful/graphql'
          ...
          <Container maxW="container.lg">
            <Box mb={16}>
              <Box textStyle="h1" mb={8}>
                <h1>{posts[0] ? `${platform.displayName}の記事一覧` : `${platform.displayName}の記事はありません`}</h1>
              </Box>
...
export async function getStaticProps({ params, preview = false }: GSProps) {
  ...
  platformData ? posts = await getAllPostsForPlatform(platformData.slug, preview, TOTAL_LIMIT) : posts = []
  return {
    props: {
